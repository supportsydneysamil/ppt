...
function addSubtitlePanel(pptx, slide, content, theme) {
  slide.addShape(pptx.ShapeType.rect, {
    ...
    line: { color: theme.border, width: theme.borderWidth || 1 },
  });

  if (theme.doubleLines) {
    [SUBTITLE_PANEL.y + 0.09, SUBTITLE_PANEL.y + SUBTITLE_PANEL.h - 0.09].forEach(
      (y, index) => {
        slide.addShape(pptx.ShapeType.line, {
          x: SUBTITLE_PANEL.x + 0.16,
          y,
          w: SUBTITLE_PANEL.w - 0.32,
          h: 0,
          objectName: `custom-title:subtitle-inner-line-${index + 1}`,
