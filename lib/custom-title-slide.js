...
function stackTitleBlocks(content, design) {
  ...
  if (content.subtitle) {
    design.drawSubtitlePanel();
  }
}

function addSubtitlePanel(pptx, slide, content, theme) {
  slide.addShape(pptx.ShapeType.rect, {
    ...SUBTITLE_PANEL,
    objectName: "custom-title:subtitle-panel",
    fill: {
      color: theme.fill,
      transparency: theme.transparency || 0,
    },
    line: { color: theme.fill, transparency: 100 },
  });

  if (theme.accent === "line") {
    slide.addShape(pptx.ShapeType.line, {
      x: LAYOUT.width / 2 - 0.55,
      y: SUBTITLE_PANEL.y + 0.04,
      w: 1.1,
      h: 0,
      objectName: "custom-title:subtitle-accent",
      line: { color: theme.accentColor, width: 1 },
    });
  }

  if (theme.accent === "dots") {
    [SUBTITLE_PANEL.x + 0.35, SUBTITLE_PANEL.x + SUBTITLE_PANEL.w - 0.41].forEach(
      (x, index) => {
        slide.addShape(pptx.ShapeType.ellipse, {
          x,
          y: SUBTITLE_PANEL.y + SUBTITLE_PANEL.h / 2 - 0.03,
          w: 0.06,
          h: 0.06,
          objectName: `custom-title:subtitle-dot-${index + 1}`,
  ...
  if (theme.accent === "diamonds") {
    [
      SUBTITLE_PANEL.x - 0.07,
      SUBTITLE_PANEL.x + SUBTITLE_PANEL.w - 0.07,
    ].forEach((x, index) => {
      slide.addShape(pptx.ShapeType.diamond, {
        x,
        y: SUBTITLE_PANEL.y + SUBTITLE_PANEL.h / 2 - 0.07,
        w: 0.14,
        h: 0.14,
        objectName: `custom-title:subtitle-diamond-${index + 1}`,
        fill: { color: theme.accentColor },
        line: { color: theme.accentColor, transparency: 100 },
      });
    });
...
function addAuroraSlide(pptx, content) {
  ...
  stackTitleBlocks(content, {
    ...
    drawSubtitlePanel: () =>
      addSubtitlePanel(pptx, slide, content, {
        fill: AURORA.bg,
        text: "FFFFFF",
        transparency: 22,
      }),
