...
export function isSlideAtResetDefaults(slide) {
  if (!slide) return true;
  const defaults = buildResetSlideDraft(slide);
  return (
    !slide.pendingFile &&
    !slide.pendingBackgroundFile &&
    !slide.pendingScriptureImage &&
    Object.keys(defaults).every((key) =>
      resetValuesMatch(slide[key], defaults[key])
    )
  );
}
...
export function canApplyResetDraft({
  ...
  return (
    Boolean(expectedSlideId) &&
    expectedSlideId === currentSlideId &&
    !isSaveBusy(saveState)
  );
}
