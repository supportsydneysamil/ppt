...
/** Metadata-only marker left in exported fields when a local asset was stripped. */
...
function shouldStripImageSrc(sourceSrc, canonicalSrc) {
  return (
    isEmbeddedAsset(sourceSrc) ||
    isStrippedAssetMarker(sourceSrc) ||
    isEmbeddedAsset(canonicalSrc) ||
    isStrippedAssetMarker(canonicalSrc)
  );
}
...
export function toPortableSlide(slide) {
  ...
  if (
    isEmbeddedAsset(source.serverFilePath) ||
    isStrippedAssetMarker(source.serverFilePath)
  ) {
    portable.serverFilePath = STRIPPED_ASSET_MARKER;
  } else {
...
export function isUnrestorableSlide(slide) {
  ...
  const localUploadPath =
    isEmbeddedAsset(slide.serverFilePath) ||
    isStrippedAssetMarker(slide.serverFilePath);
