...
import { coverCropWindow } from "../public/custom-image-layout.js";
...
function coverGeometry(box, dimensions, element) {
  ...
  const scaleX = box.w / crop.width;
  const scaleY = box.h / crop.height;
  return {
    x: box.x,
    y: box.y,
...
async function addImage(slide, element, options) {
  ...
  try {
    ...
    if (!dimensions) {
      ...
      slide.addImage({ ...common, ...coverGeometry(box, dimensions, element) });
    }
  } catch (error) {
