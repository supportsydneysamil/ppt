...
/** Blank preview dates use today; malformed non-empty values stay invalid. */
export function previewServiceDate(storedIso, todayIso) {
  const stored = typeof storedIso === "string" ? storedIso.trim() : "";
  return stored || todayIso || todayIsoDate();
}

/**
 * Resolves the date shown by the editor and refreshes the backing record only
 * for automatic modes. Custom records remain untouched, including an explicit
 * blank, while callers still receive today's display fallback.
 */
export function syncAutomaticServiceDate(slide, todayIso) {
  const mode = normalizeDateMode(slide?.dateMode);
  const resolved = resolveServiceDate(mode, slide?.serviceDate, todayIso);
  if (slide && typeof slide === "object" && mode !== "custom") {
    slide.serviceDate = resolved;
  }
  return resolved;
}
